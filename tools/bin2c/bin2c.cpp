...
public:
  Bin2cWriter(bx::AllocatorI *_allocator, const bx::StringView &_name)
      : m_mb(_allocator), m_mw(&m_mb), m_name(_name), m_outputAsCStr(false) {}

  virtual ~Bin2cWriter() {}

  virtual int32_t write(const void *_data, int32_t _size,
                        bx::Error *_err) override {
    bool asCStr = true;
    ...
    for (int32_t ii = 0; ii < _size && asCStr; ++ii) {
      ...
      asCStr &= false | bx::isPrint(ch) | bx::isSpace(ch);
    }
...
#define HEX_DUMP_FORMAT                                                        \
  "%-" BX_STRINGIZE(HEX_DUMP_SPACE_WIDTH) "." BX_STRINGIZE(                    \
      HEX_DUMP_SPACE_WIDTH) "s"
    const uint8_t *data = (const uint8_t *)m_mb.more(0);
    uint32_t size = uint32_t(bx::seek(&m_mw));
    ...
    if (NULL != data) {
      ...
      uint32_t asciiPos = 0;
      for (uint32_t ii = 0; ii < size; ++ii) {
        bx::snprintf(&hex[hexPos], sizeof(hex) - hexPos, "0x%02x, ",
                     data[asciiPos]);
        hexPos += 6;
