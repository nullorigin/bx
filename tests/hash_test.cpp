...
const HashTest s_hashTest[] = {
    //  Crc32                               | Adler32   | Murmur2A  | Murmur3 |
    //  Input Ieee        Castagnoli  Koopman     |           |           | |
    {{0, 0, 0}, 1, 0, 0, ""},
    {{0xe8b7be43, 0xc1d04330, 0x0da2aa8a},
     0x00620062,
     0x0803888b,
     0x3c2569b2,
     "a"},
    {{0x9e83486d, 0xe2a22936, 0x31ec935a},
     0x012600c4,
     0x618515af,
     0x9bbfd75f,
     "ab"},
    {{0xc340daab, 0x49e1b6e3, 0x945a1e78},
     0x06060205,
     0x94e3dc4d,
     0x1e661875,
     "abvgd"},
    {{0x07642fe2, 0x45a04162, 0x3d4bf72d},
     0x020a00d6,
     0xe602fc07,
     0x7af40d31,
     "1389"},
    {{0x26d75737, 0xb73d7b80, 0xd524eb40},
     0x04530139,
     0x58d37863,
     0x0c090160,
     "555333"},
...
TEST_CASE("HashMurmur2A", "[hash]") {
  ...
  for (uint32_t ii = 0; ii < BX_COUNTOF(s_hashTest); ++ii) {
    ...
    REQUIRE(test.murmur2a ==
            MurmurHash2A(test.input, bx::strLen(test.input), seed));
  }
}
...
TEST_CASE("HashMurmur3", "[hash]") {
  ...
  for (uint32_t ii = 0; ii < BX_COUNTOF(s_hashTest); ++ii) {
    ...
    const uint32_t sanity =
        MurmurHash3_x86_32(test.input, bx::strLen(test.input), seed);
...
TEST_CASE("HashMurmur3-Separate-Add", "[hash]") {
  ...
  hash.add("1389");
  hash.add("555333");
  REQUIRE(MurmurHash3_x86_32("0123456789abvgd0123451389555333", 31, 0) ==
          hash.end());
}
