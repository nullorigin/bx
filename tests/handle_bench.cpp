...
int main() {
  ...
  //
  {
    ...
    for (uint32_t ii = 0; ii < numIterations; ++ii) {
      ...
      for (uint32_t jj = 0; jj < numElements; ++jj) {
        tinystl::pair<TinyStlUnorderedMap::iterator, bool> ok =
            map.insert(tinystl::make_pair(uint64_t(jj), uint16_t(jj)));
        assert(ok.second);
        BX_UNUSED(ok);
      }
  ...
  ///
  {
    ...
    for (uint32_t ii = 0; ii < numIterations; ++ii) {
      ...
      map.reserve(numElements);
      for (uint32_t jj = 0; jj < numElements; ++jj) {
        std::pair<StdUnorderedMap::iterator, bool> ok =
            map.insert(std::make_pair(uint64_t(jj), uint16_t(jj)));
        assert(ok.second);
        BX_UNUSED(ok);
      }
  ...
  ///
  {
    ...
    for (uint32_t ii = 0; ii < numIterations; ++ii) {
      typedef bx::HandleHashMapT<numElements + numElements / 2, uint64_t>
          HandleHashMap;
      HandleHashMap map;
      for (uint32_t jj = 0; jj < numElements; ++jj) {
