...
static const UrlTest s_urlTest[] = {
    {true,
     "scheme://username:password@host.rs:80/this/is/path/"
     "index.php?query=\"value\"#fragment",
     {"scheme", "username", "password", "host.rs", "80",
  ...
  for (uint32_t ii = 0; ii < BX_COUNTOF(s_urlTest); ++ii) {
    ...
    if (result) {
      for (uint32_t token = 0; token < bx::UrlView::Count; ++token) {
        //				char tmp[1024];
        //				strCopy(tmp, BX_COUNTOF(tmp),
        //url.get(bx::UrlView::Enum(token)) ); 				printf("`%s`, expected: `%s`\n",
        //tmp, urlTest.tokens[token]);

        REQUIRE(0 == bx::strCmp(urlTest.tokens[token],
                                url.get(bx::UrlView::Enum(token))));
      }
    }
  }
}
