...
TEST_CASE("pow", "[math][libm]") {
  ...
  for (float xx = -100.0f; xx < 100.0f; xx += 0.1f) {
    bx::write(writer, &err, "pow(1.389f, %f) == %f (expected: %f)\n", xx,
              bx::pow(1.389f, xx), ::powf(1.389f, xx));
    REQUIRE(err.isOk());
...
TEST_CASE("bitsToFloat, floatToBits, bitsToDouble, doubleToBits", "[math]") {
  REQUIRE(UINT32_C(0x12345678) ==
          bx::floatToBits(bx::bitsToFloat(UINT32_C(0x12345678))));
  REQUIRE(UINT64_C(0x123456789abcdef0) ==
          bx::doubleToBits(bx::bitsToDouble(UINT32_C(0x123456789abcdef0))));
}
...
void mtxCheck(const float *_a, const float *_b) {
  if (!bx::isEqual(_a, _b, 16, 0.01f)) {
    DBG("\n"
        ...
        _a[10], _a[11], _a[12], _a[13], _a[14], _a[15], _b[0], _b[1], _b[2],
        _b[3], _b[4], _b[5], _b[6], _b[7], _b[8], _b[9], _b[10], _b[11], _b[12],
        _b[13], _b[14], _b[15]);
...
TEST_CASE("limits", "[math]") {
  ...
  STATIC_REQUIRE(bx::LimitsT<float>::min ==
                 std::numeric_limits<float>::lowest());
  STATIC_REQUIRE(bx::LimitsT<float>::max == std::numeric_limits<float>::max());

  STATIC_REQUIRE(bx::LimitsT<double>::min ==
                 std::numeric_limits<double>::lowest());
  STATIC_REQUIRE(bx::LimitsT<double>::max ==
