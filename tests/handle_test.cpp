...
TEST_CASE("HandleAllocT", "") {
  ...
  REQUIRE(sizeof(alloc) ==
          sizeof(uint16_t) * kMax * 2 + sizeof(bx::HandleAlloc));
...
TEST_CASE("HandleListT", "") {
  ...
  list.pushFront(15);
  uint16_t count = 0;
  for (uint16_t it = list.getFront(); it != UINT16_MAX;
       it = list.getNext(it), ++count) {
...
TEST_CASE("HandleAllocLruT", "") {
  ...
  uint16_t count = 0;
  for (uint16_t it = lru.getFront(); it != UINT16_MAX;
       it = lru.getNext(it), ++count) {
    REQUIRE(it == expected0[count]);
  }
}
